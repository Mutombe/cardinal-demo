...
# Single-tenant (Cardinal only). django-tenants removed — one Postgres schema.
INSTALLED_APPS = [
    ...
    'apps.developments',  # Property development domain (developers, stands, buyers, agreements)
]
...
MIDDLEWARE = [
    ...
    'middleware.current_user.CurrentUserMiddleware',  # thread-local user for audit trails
]
...
# Database — standard PostgreSQL (Neon), single schema. Parsed from DATABASE_URL.
tmpPostgres = urlparse(os.getenv("DATABASE_URL"))
_db_options = dict(parse_qsl(tmpPostgres.query))  # sslmode (Neon)
# This Neon DB is shared with another app in `public`; isolate the ERP in its
# own schema. Requires the UNPOOLED Neon endpoint (pooler rejects this option).
_db_schema = os.getenv("DB_SCHEMA", "cardinal")
