...
def schema_context(*args, **kwargs):
    """Vestigial no-op (single-tenant). Buyer-portal queries run in the one schema."""
    yield
...
class AuthViewSet(viewsets.ViewSet):
    ...
    def login(self, request):
        try:
            ...
            # Update last activity (single-tenant — no schema resolution)
            user.last_activity = timezone.now()
            user.save(update_fields=['last_activity'])

            tenant = None  # single-tenant: no per-request tenant object
...
class UserActivityViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        return UserActivity.objects.select_related('user')
...
class UserInvitationViewSet(viewsets.ModelViewSet):
    ...
    def _send_invitation_email(self, invitation, request):
        ...
        company_name = 'Cardinal Properties'
